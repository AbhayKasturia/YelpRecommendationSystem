...
\begin{document}
	...
	\section{Methods}
		...
		\subsection{Deep Learing Method} 
		We have using Neural Collaborative Filtering \cite{ncf} (NCF) approach suggested by Xiangnan He and team. We looked at various deep learning approaches towards building recommendation systems and choose this work as it was generally applicable compared to other techniques which either target music or video recommendations specifically. NCF uses multi-layers percpetron to learn use-item interaction function. It is generic and tries to generalize matric factorization under its framework. This work focuses on the implicit feedback which indirectly reflects users behaviours.
		...
		A multi-layer reprentation is used to model user-item interaction $y_{ui}$ as shown in below figure.
		\begin{figure}[H]
			\label{fig:ncf}
			\centering
		...
		\end{figure}
		The bottom input layer consists of two feature vectors $v_{u}^{U}$ and $v_{i}^{I}$ that describe user u and item i, respectively. As it is purely colloborative model, only user and item ids are taken as input in form of one-hot vectors. Above the input layer is the embedding layer; it is a fully connected layer that projects the sparse representation to a dense vector. The user embedding and item embedding are then fed into a multi-layer perceptron architecture to map the latent vectors to prediction scores.  The final output layer is the predicted score $\hat{y_{ui}}$, and training is performed by minimizing the pointwise loss between $\hat{y_{ui}}$
